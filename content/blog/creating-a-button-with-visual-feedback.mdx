...
Next the basic styles are added to the component using Styled Components.

The styles for the button are added to the  `StyledButton` styled component. There are also styles for the button for it's hovered and disabled states.
...
At this point you should have a button that looks like the one below. If you put the button to be disabled it will look like the second button.

Go ahead and give it a press.
...
<Button style={{marginLeft : 10 + 'px'}}  disabled onClick={() => alert('Hey you pressed me')}>Button</Button>
```
The next step will adding a loading state to the button.

## Adding a loading state to the button

Next add the styles following styles to the `Styled Button` constant.
...
Then pass the **isLoading** property to the disabled attribute of the button so the button will be disabled when it is loading to prevent secondary clicks.
...
For the icon we importing **FontAwesomeIcon** from the `'@fortawesome/react-fontawesome'` package and using the **faSpinner** icon from the `'@fortawesome/free-solid-svg-icons'` package. To make the icon spin add the property of spin to the **FontAwesomeIcon** component.
...
To add a completed state to the button 
